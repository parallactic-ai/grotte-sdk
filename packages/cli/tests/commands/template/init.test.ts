...
describe('Template Init', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = await fs.mkdtemp('grotte-init-test-')
  })
  ...
  describe('Package.json Integration', () => {
    test('should add scripts to existing package.json', async () => {
      ...
      expect(updatedPackageJson.scripts).toHaveProperty('grotte:build:dev')
      expect(updatedPackageJson.scripts).toHaveProperty('grotte:build:prod')
    ...
    test('should work without package.json', async () => {
      ...
      expect(createdPackageJSON.scripts).toHaveProperty('grotte:build:dev')
      expect(createdPackageJSON.scripts).toHaveProperty('grotte:build:prod')
    })
  })
...
\tCGO_ENABLED=1 go build
      ...
      expect(updatedMakefile).toContain('grotte:build:dev')
      expect(updatedMakefile).toContain('grotte:build:prod')
...
\tCGO_ENABLED=1 go build`) // existing script preserved
    ...
    test('should work without Makefile', async () => {
      ...
      expect(createdMakefile).toContain('grotte:build:dev')
      expect(createdMakefile).toContain('grotte:build:prod')
    })
  })
  ...
  describe('File Content Validation', () => {
    test('should generate correct TypeScript template content', async () => {
      ...
      expect(templateContent).toContain("import { Template } from 'grotte'")
      expect(templateContent).toContain('export const template = Template()')
      expect(templateContent).toContain('fromImage')
      expect(templateContent).toContain('parallactic/base')
    })
    ...
    test('should generate correct Python template content', async () => {
      ...
      // Verify basic structure
      expect(templateContent).toContain('from grotte import Template')
    ...
    test('should generate correct async Python template content', async () => {
      ...
      expect(templateContent).toContain('from grotte import AsyncTemplate')
