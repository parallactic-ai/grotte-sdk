...
from grotte.api.metadata import default_headers
...
limits = Limits(
    max_keepalive_connections=int(os.getenv("GROTTE_MAX_KEEPALIVE_CONNECTIONS", "20")),
    max_connections=int(os.getenv("GROTTE_MAX_CONNECTIONS", "2000")),
    keepalive_expiry=int(os.getenv("GROTTE_KEEPALIVE_EXPIRY", "300")),
)
...
class ApiClient(AuthenticatedClient):
    """
    The client for interacting with the GROTTE API.
    """
        ...
        **kwargs,
        ...
        if require_api_key:
            if config.api_key is None:
                raise AuthenticationException(
                    "API key is required, please visit the Team tab at https://app.grotte.dev/dashboard to get your API key. "
                    "You can either set the environment variable `GROTTE_API_KEY` "
                    'or you can pass it directly to the method like api_key="grt_..."',
                )
        ...
        if require_access_token:
            if config.access_token is None:
                raise AuthenticationException(
                    "Access token is required, please visit the Personal tab at https://app.grotte.dev/dashboard to get your access token. "
                    "You can set the environment variable `GROTTE_ACCESS_TOKEN` or pass the `access_token` in options.",
                )
