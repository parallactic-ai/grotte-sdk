...
import grt_connect
from grotte.connection_config import (
...
from grt_connect.client import Code

from grotte.envd.api import ENVD_API_FILES_ROUTE, handle_envd_api_exception
from grotte.envd.filesystem import filesystem_connect, filesystem_pb2
from grotte.envd.rpc import authentication_header, handle_rpc_exception
from grotte.envd.versions import (
    ENVD_DEFAULT_USER,
...
from grotte.sandbox.filesystem.filesystem import (
    EntryInfo,
    WriteEntry,
    WriteInfo,
    map_file_type,
...
from grotte.sandbox_sync.filesystem.watch_handle import WatchHandle
...
class Filesystem:
    ...
    def __init__(
        ...
        self._rpc = filesystem_connect.FilesystemClient(
            ...
            # compressor=grt_connect.GzipCompressor,
            pool=pool,
            json=True,
    ...
    def exists(
        ...
        try:
            ...
            if isinstance(e, grt_connect.ConnectException):
                if e.status == grt_connect.Code.not_found:
                    return False
            raise _handle_filesystem_rpc_exception(e)
    ...
    def make_dir(
        ...
        try:
            ...
            if isinstance(e, grt_connect.ConnectException):
                if e.status == grt_connect.Code.already_exists:
                    return False
            raise _handle_filesystem_rpc_exception(e)
