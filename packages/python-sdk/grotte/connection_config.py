...
from grotte.api.metadata import package_version
...
class ApiParams(TypedDict, total=False):
    ...
    """GROTTE API Key to use for authentication, defaults to `GROTTE_API_KEY` environment variable."""

    domain: Optional[str]
    """GROTTE domain to use for authentication, defaults to `GROTTE_DOMAIN` environment variable."""
    ...
    """URL to connect to sandbox, defaults to `GROTTE_SANDBOX_URL` environment variable."""
...
class ConnectionConfig:
    ...
    @staticmethod
    def _debug():
        return os.getenv("GROTTE_DEBUG", "false").lower() == "true"

    @staticmethod
    def _api_key():
        return os.getenv("GROTTE_API_KEY")

    @staticmethod
    def _api_url():
        return os.getenv("GROTTE_API_URL")

    @staticmethod
    def _sandbox_url():
        return os.getenv("GROTTE_SANDBOX_URL")

    @staticmethod
    def _access_token():
        return os.getenv("GROTTE_ACCESS_TOKEN")
    ...
    def __init__(
        ...
        self.headers["User-Agent"] = f"grotte-python-sdk/{package_version}"
    ...
    def sandbox_headers(self):
        """
        We need this separate as we use the same header for GROTTE access token to API and envd access token to sandbox.
        """
        return {
            **self.headers,
